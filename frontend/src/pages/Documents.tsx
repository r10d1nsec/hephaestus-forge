...
const DOC_ORDER = ["blueprint", "prd", "tech_spec", "estimation"];
...
export default function Documents() {
  ...
  const docLabel = (dt: string) => t(`docs.label.${dt}`);
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="w-56 border-r border-forge-border bg-forge-panel p-4">
        ...
        {DOC_ORDER.map((dt) => (
          <button
            ...
            className={`mb-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm ${
              selected === dt ? "bg-forge-ember/15 text-forge-ember-soft" : "text-forge-steel hover:bg-forge-border/50"
            }`}
          >
            <FileText size={15} />
            {docLabel(dt)}
            {generating && !contents[dt] && <Loader2 size={12} className="ml-auto animate-spin" />}
          </button>
        ))}
        <a
        ...
        >
          <Download size={15} /> {t("docs.exportZip")}
        </a>
        {!generating && (
          <button
      ...
      <div className="flex-1 overflow-y-auto px-10 py-8">
        {generating && !contents[selected] ? (
          <div className="flex items-center gap-2 text-forge-steel">
            <Loader2 size={16} className="animate-spin" /> {t("docs.forging", { doc: docLabel(selected) })}
          </div>
        ) : contents[selected] ? (
          ...
          <div className="text-forge-steel">
            {t("docs.noContent", { action: t("docs.regenerate") })}
          </div>
        )}
      </div>
    </div>
